...
def _is_binary_target(target):
    ...
    Returns:
        A boolean indicating whether the target is a binary target.
    """
    return target["module_type"] == module_types.binary_target

def _is_binary_product(target):
    """Returns True if the specified target is a binary target. Otherwise False.
    ...
    Returns:
        A boolean indicating whether the target is a binary target.
    """
    return target["type"] == target_types.binary
...
package_descriptions = struct(
    ...
    is_binary_product = _is_binary_product,
